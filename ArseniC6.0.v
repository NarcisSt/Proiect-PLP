...
Definition EnvB := Var -> ErrorBool.
Definition env5 := fun x => if string_dec x "x" then false else true.

Fixpoint interpret (e : BExp1) (env : EnvB) : ErrorBool :=
  match e with
  | bTrue => true
  | bFalse => false
  | bVar x => (env x)
  | bNot x => not_ErrorBool (interpret x env)
  | bAnd e1 e2 => and_ErrorBool (interpret e1 env) (interpret e2 env) 
  | bOr e1 e2 => or_ErrorBool (interpret e1 env) (interpret e2 env)
  end.

Compute (interpret bFalse env5).

(* Define a stack machine *)

Inductive BInstruction :=
| push_const : ErrorBool -> BInstruction
| push_var : Var -> BInstruction
| nu : BInstruction
| si : BInstruction
| sau : BInstruction.

Definition BStack := list ErrorBool.

Definition run_instruction (i : BInstruction)(env : EnvB) (stack : BStack) : BStack :=
  match i with
  | push_const c => (c :: stack)
  | push_var x => ((env x) :: stack)
  | nu => match stack with
           | n1 :: stack' => ( not_ErrorBool n1) :: stack'
           | _ => stack
           end  
  | si => match stack with
           | n1 :: n2 :: stack' => ( and_ErrorBool n1 n2) :: stack'
           | _ => stack
           end 
  | sau => match stack with
           | n1 :: n2 :: stack' => ( or_ErrorBool n1 n2) :: stack'
           | _ => stack
           end
  end.

Compute (run_instruction (push_const true) env5 []).
Compute (run_instruction (push_var "x") env5 []).


Fixpoint run_instructions (is' : list BInstruction) (env : EnvB) (stack : BStack) : BStack :=
  match is' with
  | [] => stack
...
Compute run_instructions pgm1 env5 [].
...
Fixpoint compile (e : BExp1) : list BInstruction :=
  ...
  | bVar x => [push_var x]
  | bNot x => (compile x ) ++ [nu]
  | bAnd e2 e1 => (compile e1) ++ (compile e2) ++ [si]
  | bOr e2 e1 => (compile e1) ++ (compile e2) ++ [sau]
  end.
...
Compute interpret (!!!'(bFalse |||' (bVar "x") &&&' bTrue)) env5.
Compute run_instructions (compile (!!!'(bFalse |||' (bVar "x") &&&' bTrue))) env5 [].
